...
angular.module('standard').controller('ctrl.leftnav', function ($scope, $element) {
  ...
  $scope.leftMenus = [{
    ...
    },
    {
        'name': '用户分析',
        'sign': 'user',
        'imgpath': '/img/nav/user-analysis.png',
        'subs': [
    ...
    },
    {
        'name': '事件与转化',
        'sign': 'eventantrans',
        'imgpath': '/img/nav/event-and-trans.png',
        'subs': [
    ...
    },
    {
        'name': '用户参与度',
        'sign': 'userpartin',
        'imgpath': '/img/nav/participation.png',
        'subs': [
  ...
  $scope.clickFirstMenu = function(e) {
    ...
    if ($($element).find("#report" + e).find("div.menu_body").is(":hidden")) {
        $($element).find("#report" + e).find('.icon-down').addClass('close').removeClass('open');
        ...
        $($element).find("#report" + e).find('.icon-down').addClass('open').removeClass('close');
